    ...
    <div class="container py-8">
      <div class="text-center mb-8">
        ...
        <p class="text-text-secondary text-lg max-w-2xl mx-auto">
          Monitor package ecosystems for end-of-life components and security vulnerabilities. 
          Get extended support through HeroDevs NES.
        </p>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @for (ecosystem of ecosystemSummary(); track ecosystem.ecosystem) {
          <a [routerLink]="[ecosystem.ecosystem]" class="card block hover:shadow-lg transition-shadow">
            ...
            <div class="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <div class="text-lg font-semibold text-success">{{ ecosystem.heroDevsPackageCount }}</div>
              </div>
            </div>

            @if (ecosystem.vulnerabilitiesBySeverity && Object.keys(ecosystem.vulnerabilitiesBySeverity).length > 0) {
              <div class="mt-4 pt-4 border-t border-border-color">
                ...
                <div class="flex gap-2 flex-wrap">
                  ...
                  @if (ecosystem.vulnerabilitiesBySeverity['high']) {
                    <span class="inline-flex items-center px-2 py-1 rounded text-xs bg-orange-500 text-white">
                      High: {{ ecosystem.vulnerabilitiesBySeverity['high'] }}
                    </span>
                  }
                  @if (ecosystem.vulnerabilitiesBySeverity['medium']) {
                    <span class="inline-flex items-center px-2 py-1 rounded text-xs bg-warning text-neutral-900">
                      Medium: {{ ecosystem.vulnerabilitiesBySeverity['medium'] }}
                    </span>
                  }
                  @if (ecosystem.vulnerabilitiesBySeverity['low']) {
                    <span class="inline-flex items-center px-2 py-1 rounded text-xs bg-neutral-400 text-white">
