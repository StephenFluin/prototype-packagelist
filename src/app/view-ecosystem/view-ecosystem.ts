    ...
    <div class="container py-8">
      ...
      <div class="mb-8">
        ...
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-3xl font-bold capitalize mb-2">{{ ecosystemName() }} Packages</h1>
            <p class="text-text-secondary">
              {{ packages().length }} packages found @if (filteredPackages().length !==
              packages().length) {
      ...
      <div class="text-center py-12">
        ...
        <p class="text-text-muted">
          @if (searchTerm()) { No packages match your search criteria. } @else { This ecosystem
          doesn't have any packages yet. }
        </p>
      </div>
      }
    </div>
...
export class ViewEcosystem {
  ...
  filteredPackages = computed(() => {
    ...
    if (search) {
      filtered = filtered.filter((pkg) => pkg.name.toLowerCase().includes(search));
    }
