              ...
              <div class="flex gap-1 mt-2">
                @for (severity of ['critical', 'high', 'medium', 'low']; track severity) {
                  @if (vulnerabilitiesBySeverity()[severity]) {
                    <span [class]="getSeverityDotClass(severity)" class="w-2 h-2 rounded-full" [title]="severity + ': ' + vulnerabilitiesBySeverity()[severity]"></span>
                  }
                }
              </div>
            }
...
export class ViewPackage {
  ...
  packageName = toSignal(this.route.params.pipe(map(params => params['package'])), { initialValue: '' });
  
  packageData = computed(() => {
    const ecosystem = this.ecosystemName();
    const packageName = this.packageName();
    return ecosystem && packageName ? this.dataService.getPackage(ecosystem, packageName)() : undefined;
  });
